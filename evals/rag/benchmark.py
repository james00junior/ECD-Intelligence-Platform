...
def evaluate_case(case: dict, db) -> dict:
    ...
    # Evaluate documents rather than chunks: several retrieved chunks from the
    # same document should not artificially improve the ranking metric.
    retrieved = [uri for uri in (_source_uri(item) for item in evidence) if uri]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate internal RAG retrieval.")
    ...
    with SessionLocal() as db:
        for case in dataset:
            ...
            status = "✓" if result["recall_at_5"] else "✗"
            print(
                f"{status} {result['id']}: "
